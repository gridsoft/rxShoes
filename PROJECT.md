...
| 2026-09-21 | Decision | **2-pack discount setting + one source of truth for every discount number.** Appearance > Customize > Bundle Discount now has two fields: 3-pack (`rx_bundle_max_discount_percent`, default 45) and 2-pack (`rx_bundle_two_pack_discount_percent`, default **35** — the client's stated number; it matches neither Figma's 30 nor its 40, see TODO.md #3). Both are *option*-type settings (same reasoning as before: survive a theme switch, readable by rx-core), read via `rx_theme_bundle_max_discount_percent()` / `rx_theme_bundle_two_pack_discount_percent()`, each with a filter for the pricing engine. A validator rejects a 2-pack discount bigger than the 3-pack one (cards advertise the 3-pack as the best price). **Supersedes the "homepage tier text does not follow this setting" note in the product-card follow-up entry:** homepage copy (Hero tier badges, Power Rotation tier tags/status lines/toggle labels/calculator) now uses placeholders — `{two_pack}`, `{three_pack}`, and for the calculator's example basket `{three_pack_total}`, `{three_pack_savings}` (+ the `two_pack` equivalents; base = the number in the calculator's "original total" field). Substitution happens inside `rx_theme_get_mod()`, so templates and Customizer live-preview partials both get it; the Hero/Power Rotation section descriptions explain the placeholders. Verified: defaults render 35/45 with "You Save $292.50 AUD (45%)" (matches Figma's own maths); setting 40/50 changes hero, rotation, calculator (→ $325.00) and shop cards ("As low as", "+50% tier") together; the shop card only ever displays the 3-pack tier (no Figma element for the 2-pack yet). No text overrides had been saved, so changing the defaults from Figma's mixed 30/40/45/55 to tokens took effect immediately — visible change from the Figma copy, intended. |
| 2026-09-21 | Issue | **Customizer Publish crashed (HTTP 500) with two cross-validating settings.** The 2-pack ≤ 3-pack validator read the other setting with `WP_Customize_Setting::post_value()`, which itself *validates* that setting, whose validator called `post_value()` on the first… infinite mutual recursion (Xdebug's 256-frame limit surfaced it as a "critical error"; plain PHP would exhaust the stack). Found only because the save was tried in a real browser — the validation message alone looked fine. **Fix / rule:** inside a `validate_callback`, never call `post_value()` on another setting; read `$manager->unsanitized_post_values()` and fall back to the stored option. Verified in the real Customizer: 50/40 publishes and persists; 2-pack 60 (> 3-pack 50) is rejected with one field changed; both fields changed and still invalid → both flagged; correcting the pair publishes. |
| 2026-09-21 | Decision | **"Best for" is now a real product taxonomy, not a textarea — supersedes point (3) of the product-card follow-up entry (I had misread the request).** `RXCoreCatalogBestForTaxonomy` registers `rx_best_for` on `product`: managed under Products > Best for, and on the product edit screen it is a tick-box list in the side column exactly like Product categories (`hierarchical => true` is what gives the tick-box UI; the terms are a flat list). It's in rx-core, not the theme, because it's catalogue data that must survive a theme switch. Not public (no archive URLs) but REST-enabled; uses WooCommerce's `*_product_terms` capabilities. It shows for variable products (it's a side metabox, not a product-data tab), so nothing goes in the Shop card tab for it. The textarea and its `_rx_best_for` meta are gone; the theme's `rx_theme_product_best_for()` joins the ticked term names with " • ". Filtering the shop by "Best for" is now possible later. **Dev data:** the four test products' old textarea lines were converted to terms (8 terms; product 55 got "All-Rounder, Gym Training", etc.). Verified in a real browser on a variable product: box visible with 9 terms, old textarea absent, a genuine click-Update saved a changed selection, the shop card showed it, restored afterwards. PHPCS clean; PHPStan needs `--memory-limit=1G` with the WooCommerce stubs (default 256M crashes the worker — a resource limit, not a code error). **Process slip worth remembering:** my first migration script deleted the old meta before checking the terms existed (the plugin edit hadn't applied, so the taxonomy wasn't registered); only dev data, restored from the values I had just printed — for real data, check the target exists before deleting the source. |
