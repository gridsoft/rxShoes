...
* **Update (2026-09-21, later still):** Category Cards section built — query/logic/admin-fields layer done and verified end-to-end (see §13 dev log), styling deliberately left neutral pending that section's exact Figma node data (hit an API rate limit pulling it; background retry in progress).
* **Immediate Next Steps:**
  1. [ ] Style Category Cards once its Figma node data is available (colours/badge treatment/image aspect ratio — currently neutral/unstyled layout only)
  2. [ ] Remaining homepage sections, in Figma order: Power Rotation tiers, Best Sellers grid, Educational comparison, Shop by Brand, Community Rotations
  3. [ ] Fulfilment answer (Section 15 Q6) — only open client question left
